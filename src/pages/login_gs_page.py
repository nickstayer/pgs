...
message_incorrect_login_pass_locator = (
    By.XPATH,
    '//div[contains(text(), "Неверные логин или пароль")]',
)
...
class LoginGsPage(BasePage):
    ...
    def incorrect_enter(self):
        message_incorrect_login_pass = self.wait_and_find(
            message_incorrect_login_pass_locator, 3
        )
        message_incorrect_login = self.wait_and_find(message_incorrect_login_locator, 3)
